...
var AuthClient = class {
  ...
  async #exchangeProfileToken(options) {
    ...
    if (options.actorToken) {
      if (tokenResponse.claims?.act) {
        ...
        try {
          tokenResponse.act = (0, import_jose2.decodeJwt)(tokenEndpointResponse.access_token).act;
        } catch {
        }
      }
    }
  ...
  async verifyLogoutToken(options) {
    ...
    this.#jwks ||= (0, import_jose2.createRemoteJWKSet)(new URL(jwksUri), {
      cacheMaxAge: cacheConfig.ttlMs,
      [import_jose2.customFetch]: this.#customFetch,
      [import_jose2.jwksCache]: this.#jwksCache
    });
    const { payload } = await (0, import_jose2.jwtVerify)(options.logoutToken, this.#jwks, {
    ...
    if (!this.#clientAuthPromise) {
      this.#clientAuthPromise = (async () => {
        ...
        if (clientPrivateKey && !(clientPrivateKey instanceof CryptoKey)) {
          clientPrivateKey = await (0, import_jose2.importPKCS8)(
            clientPrivateKey,
